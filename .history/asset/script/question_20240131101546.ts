...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // questionListContainer.className = "question"
...
if (questionListContainer) {
    ...
    function displayCurrentQuestion() {
      ...
      currentQuestion.options.forEach((option, index) => {
        ...
        optionListItem.textContent = `${String.fromCharCode(65 + index)}: ${option}`;
  
        // Add a click event listener to each option
        optionListItem.addEventListener("click", () => {
          // Check if the option is already marked
          if (!optionListItem.classList.contains("marked")) {
            markOption(optionListItem, option === currentQuestion.answer);
          }
        });
      ...
      // Append the "Next" and "Submit" buttons to the right containe
