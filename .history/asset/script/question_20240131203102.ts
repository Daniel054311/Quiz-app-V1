    ...
    if (questionListContainer) {
      ...
      if (filteredData.length > 0) {
        ...
        function displayCurrentQuestion() {
         ...
         var totalScore = markAnswer(optionsList, currentQuestion.answer);
...
function markAnswer(optionsList: HTMLUListElement, correctAnswer: string) {
  ...
  optionsList.querySelectorAll('li').forEach((option) => {
    if (option.textContent?.endsWith(correctAnswer)) {
      ...
      // Mark thelet score = 0;  // Declare the score variable outside the function

function markAnswer(optionsList: HTMLUListElement, correctAnswer: string) {
  // Reset score before marking new answers
  score = 0;
