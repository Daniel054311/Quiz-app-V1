        ...
        function displayCurrentQuestion() {
          ...
          currentQuestion.options.forEach((option, index) => {
            ...
            // Create a span for the alphabet
             alphabetSpan = document.createElement("span");
  ...
  optionsList.querySelectorAll("li").forEach((option) => {
    if (option.textContent?.endsWith(correctAnswer)) {
      if (option.classList.contains("marked")) {
        ...
        score += 1;
// Get the HTMLCollectionOf<Element>
const elements = document.getElementsByClassName('letter');

// Iterate through the collection and set the style for each element
for (let i = 0; i < elements.length; i++) {
  const element = elements[i] as HTMLElement; // Use type assertion to treat it as HTMLElement
