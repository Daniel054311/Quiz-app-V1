...
function markAnswer(optionsList: HTMLUListElement, correctAnswer: string) {
  ...
  if (!markedOption) {
    // No option marked, return
    return;
  }

  // Remove the "marked" class from the marked option
  markedOption.classList.remove("marked");

  // Check if the selected option is correct
  if (markedOption.textContent?.endsWith(correctAnswer)) {
    // Mark the selected option as correct
    markedOption.style.border = '2px solid green';
    markedOption.classList.add('correct');
    ...
    markedOption.classList.add('wrong');
  }
}
