...
function markOption(optionElement: HTMLElement, isCorrect: boolean) {
    // Block user from selecting again if the option is already marked
    if (optionElement.classList.contains('marked')) {
        return;
    }

    // Remove border color from the previously selected option
    if (selectedOption) {
        selectedOption.style.border = '';
        selectedOption.classList.remove('marked');
    }
