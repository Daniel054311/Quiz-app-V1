...
function markOption(optionElement: HTMLElement, isCorrect: boolean) {
    ...
    // Update total score for correct and wrong answers
    if (isCorrect) {
...
function markAnswer(optionsList: HTMLUListElement, correctAnswer: string) {
    ...
    Array.from(optionsList.children).forEach((optionElement) => {
        if (optionElement instanceof HTMLElement && optionElement.classList.contains('marked')) {
            userSelected = true;
    ...
    // Only update total score if the user has selected an option
    if (userSelected) {
        totalScore += userSelected ? 1 : 0;
    }
}
