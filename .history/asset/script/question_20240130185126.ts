...
if (questionListContainer) {
    ...
    function displayCurrentQuestion() {
        ...
        submitButton.addEventListener('click', () => {
            // You can perform additional actions here before moving to the next question
            // For example, you might want to show a loading spinner before marking options
            setTimeout(() => {
                // Mark the correct answer (if not already marked)
                markOption(optionsList.children[currentQuestion.options.indexOf(currentQuestion.answer)], true);

                // Show the "Next" button
                showNextButton();
            }, 1000); // Wait for 1 second (adjust the time as needed)
        });
