...
interface ApiResponse1 {
  ...
  // Initialize current page index and total questions
  let currentPage = 0;
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      ...
      if (leftContainer && questionsContainer) {
        ...
        // Set total questions based on the first quiz (assuming all quizzes have the same number of questions)
        ...
        const displayQuestions = () => {
          ...
          questionElement.textContent = `Question ${
            currentPage + 1
          } of ${totalQuestions}: ${quiz.questions[currentPage].question}`;
